...
class DeleteMathCourse extends StatefulWidget {
  @override
  _DeleteMathCourseState createState() => _DeleteMathCourseState();
}

class _DeleteMathCourseState extends State<DeleteMathCourse> {
  var name,
      email,
      phone,
      firsthour,
  ...
  Future getHomeNursingDoctorInfo() async {
    try {
      var data = {"CategoryName": "Math"};
      var url = "http://" + ipAdressValue + "/HCare/OccupationalDoctor.php";
      var response = await http.post(Uri.parse(url), body: data);
      var responsebody = jsonDecode(response.body);
      // print(responsebody);
      if (responsebody['status'] == "success") return responsebody['abouts'];
  ...
  Future getHomeNursingDoctorMoreInfo(var emailf, var name) async {
    try {
      var data = {"emailf": emailf, "name": name, "types": "math"};
      var url = "http://" + ipAdressValue + "/HCare/NurseDoctorDetail.php";
      var response = await http.post(Uri.parse(url), body: data);
      var responsebody = jsonDecode(response.body);
      //print(responsebody['about']);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title:Text("All Math Teachers"),
        titleSpacing: 0,
        elevation: 0,
        backgroundColor: Theme.of(context).accentColor,),
