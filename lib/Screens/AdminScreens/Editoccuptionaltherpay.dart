...
class Editoccuptionaltherpay extends StatefulWidget {
  final currentValue;
  Editoccuptionaltherpay(
      {Key key, @required this.currentValue})
      : super(key: key);
  @override
  _EditoccuptionaltherpayState createState() => _EditoccuptionaltherpayState();
}

class _EditoccuptionaltherpayState extends State<Editoccuptionaltherpay> {
  ...
  Future<void> _pressSavePhone() async {
    ...
    if (isValid) {
      ...
      try {
        ...
        var url = "http://"+ipAdressValue+"/HCare/EditOccuptionalTherpay.php";
        var response = await http.post(Uri.parse(url), body: data);
        var responsebody = jsonDecode(response.body);
        if (responsebody['status'] == "success") {
          Navigator.of(context).pop();
          completeDonecorrect(
              context, "Edit Math about", "Edit  Math about done successfully");
        } else {
          Navigator.of(context).pop();
          showFailedDialog(context, "Edit  Math about", "Edit  Math about failed");
        }
      } catch (e) {
        Navigator.of(context).pop();
        print(e.toString());
                ...
                ListTile(
                  ...
                  subtitle: Container(
                    ...
                    child: TextFormField(
                      ...
                      validator: (value) {
                        if (value.isEmpty ) {
                          return 'Math about should not empty';
                        }
                        return null;
                      },
                    ),
