...
sendMail(femail)async{
    ...
    ..from = new Address(femail,"Learning City Technical Center")
    ..recipients.add(femail)

    // ..ccRecipients.addAll(['destCc1@example.com', 'destCc2@example.com'])
    // ..bccRecipients.add(new Address('bccAddress@example.com'))
    ..subject = 'Password recover link from Learning City  ${new DateTime.now()}'
  ...
  Future<void> _pressSignup() async {
    ...
    if (isValid) {
      ...
      if (dropdownValue == "Select type") {
            showFailedDialog(context, "", "Please select type of teacher");
      }
      else{
        try {
        var data = {
        ...
        var url = Uri.parse("http://"+ipAdressValue+"/HCare/SignUpEmployee.php");
        var response = await http.post(url, body: data);
        var responsebody = jsonDecode(response.body);
        if (responsebody['status'] == "success") {
          showFailedDialog(context, "Add Teacher", "Add Teacher done successfully");
          updateForgotPassword(_email);
        } else {
          showFailedDialog(
              context, "Add Teacher failed", "Email is already in use");
          //print("NO");
        }
      } catch (e) {
        print(e.toString());
