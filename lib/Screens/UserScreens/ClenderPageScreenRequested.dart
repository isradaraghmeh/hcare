...
import '../../Screens/UserScreens/ChangeAppointmentDetails.dart';
import '../../Forms/ConstantIP.dart';

class CelenferPageScreenRequested extends StatefulWidget {
  @override
  _CelenferPageScreenRequestedState createState() =>
      _CelenferPageScreenRequestedState();
}

class _CelenferPageScreenRequestedState
    extends State<CelenferPageScreenRequested> {
  var _currentDate;
  var _currntemail = '';
  // var _currentPhone = '';
  bool isRed = false;
  ...
  Future deleteRequest(id) async {
    try {
      var data = {"id": id};
      var url = "http://"+ipAdressValue+"/HCare/CustomerDeleteRequest.php";
      var response = await http.post(Uri.parse(url), body: data);
      var responsebody = jsonDecode(response.body);
      if (responsebody['status'] == "success") {
        // print(responsebody);
  ...
  completeDonecorrect(context, String title, String str, var id) {
    showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            ...
            actions: <Widget>[
              FlatButton(
                onPressed: () {
                  ...
                  completeDonecorrect2(context, "Delete request",
                      "Delete request done successfully");
                },
                child: Text("Yes"),
              ),
              FlatButton(
  ...
  completeDonecorrect2(context, String title, String str) {
    showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => ChangeAppointmentDetails(
                                name: name,
                                time: time,
                                region: region,
                                city: city,
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          completeDonecorrect(context, "Delete request",
                              "You are sure you to delete this request", id);
                        },
                      ),
                      // Text(
                      //   "\$50",
