...
class NaturalTherpayScreen extends StatefulWidget {
  @override
  _NaturalTherpayScreenState createState() => _NaturalTherpayScreenState();
}

class _NaturalTherpayScreenState extends State<NaturalTherpayScreen> {
  var name,
      email,
      phone,
      firsthour,
...
var filterBy = 0;
  ...
  Future getHomeNursingDoctorInfo() async {
    try {
      var data = {"CategoryName": "Arabic"};
      var url = "http://"+ipAdressValue+"/HCare/NaturalDoctor.php";
      var response = await http.post(Uri.parse(url), body: data);
      var responsebody = jsonDecode(response.body);
      // print(responsebody);
...
Future getHomeNursingDoctorInfo2() async {
    try {
      var url = "http://" + ipAdressValue + "/HCare/NaturalDoctor2.php";
  ...
  Future getHomeNursingDoctorMoreInfo(var emailf, var name) async {
    try {
      var data = {"emailf": emailf, "name": name, "types": "arabic"};
